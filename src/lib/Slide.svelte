            ...
            for (var j = 0; j < classList.length; j++) {
                if (classList[j].startsWith('start-')) {
                    if (step == 0) {
                        ...
                        const classToRemove = classList[j].split(':').slice(1).join('');
                        const stepStart = parseInt(classList[j].split(':')[0].split('-')[1]);
                        // determine the maxSteps for each slide
                        $maxSteps = stepStart > $maxSteps ? stepStart : $maxSteps;
                        // check if the class is removed by an end earlier
                        const regex = new RegExp('end-[0-' + stepStart + ']:' + classToRemove);
                        ...
                        const classToAdd = classList[j].replace(prefixStart, '');
                    ...
                    if (step == 0) {
                        ...
                        const classToAdd = classList[j].split(':').slice(1).join('');
                        const stepEnd = parseInt(classList[j].split(':')[0].split('-')[1]);
                        // determine the maxSteps for each slide
                        $maxSteps = stepEnd > $maxSteps ? stepEnd : $maxSteps;

                        // check if the class is introduce by an earlier start
                        const regex = new RegExp('start-[0-' + stepEnd + ']:' + classToAdd);
                        ...
                        const classToRemove = classList[j].replace(prefixEnd, '');
                        child.classList.remove(classToRemove);
                    }
                }
            }
